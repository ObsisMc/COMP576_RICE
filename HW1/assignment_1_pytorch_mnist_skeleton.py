...
logging_complex_interval = 100
logging_test_error_interval = 1100
logging_dir = None

# 1) setting up the logging
# logging_dir = os.path.join('runs', datetime.now().strftime('%b%d_%H-%M-%S'))
logging_dir = os.path.join('runs', "tanh_adagrad")
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % logging_complex_interval == 0:
            n_iter = (epoch - 1) * len(train_loader) + batch_idx + 1
            for name, param in model.named_parameters():
                nm = name.replace('.', '/')
                writer.add_histogram(f"{nm}", param.clone().cpu().data.numpy(), n_iter)
                writer.add_scalar(f'{nm + "/max"}', param.max().item(), n_iter)
                writer.add_scalar(f'{nm + "/min"}', param.min().item(), n_iter)
                writer.add_scalar(f'{nm + "/std"}', param.clone().reshape(-1, ).std(dim=0).item(), n_iter)
...
def test(epoch):
    ...
    for i, (data, target) in enumerate(test_loader):
        ...
        correct += correct_now
        if i % logging_test_error_interval == 0:
            test_error = 100. * (1 - correct_now / test_batch_size)
            n_iter = (epoch - 1) * len(test_loader) + test_batch_size + 1
            writer.add_scalar('test/batch error', test_error, n_iter)
    ...
    writer.add_scalar('test/error', 100.0 - test_accuracy, epoch)
